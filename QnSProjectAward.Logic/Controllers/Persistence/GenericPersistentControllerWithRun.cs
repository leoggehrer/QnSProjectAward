...
namespace QnSProjectAward.Logic.Controllers.Persistence
{
        ...
        where E : Entities.IdentityEntity, I, Contracts.ICopyable<I>, new()
    {
        ...
        protected internal IQueryable<E> QueryableSet() => Context.QueryableSet<I, E>();
        ...
        {
            return Task.Factory.StartNew(() => QueryableSet().FirstOrDefault(predicate));
        }
        internal virtual Task<E> ExecuteSingleOrDefaultAsync(Expression<Func<E, bool>> predicate)
        {
            return Task.Factory.StartNew(() => QueryableSet().SingleOrDefault(predicate));
        }
        internal virtual Task<E[]> ExecuteWhereAsync(Expression<Func<E, bool>> predicate)
        {
            return Task.Factory.StartNew(() => QueryableSet().Where(predicate).ToArray());
        }
