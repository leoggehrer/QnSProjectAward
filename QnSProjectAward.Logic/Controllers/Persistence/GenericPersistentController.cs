...
using QnSProjectAward.Logic.Modules.Exception;
...
namespace QnSProjectAward.Logic.Controllers.Persistence
{
        ...
        where E : Entities.IdentityEntity, I, Contracts.ICopyable<I>, new()
    {
        ...
        internal virtual async Task<IEnumerable<E>> ExecuteGetPageListEntitiesAsync(int pageIndex, int pageSize)
        {
            if (pageSize < 1 && pageSize > MaxPageSize)
                throw new LogicException(ErrorType.InvalidPageSize);
        ...
        {
            var qryResult = await QueryableSet().ToArrayAsync().ConfigureAwait(false);

            return await BeforeReturnAsync(qryResult).ConfigureAwait(false);
        }
        internal virtual async Task<IEnumerable<E>> ExecuteGetAllEntitiesAsync()
        {
            var qryResult = await QueryableSet().ToArrayAsync().ConfigureAwait(false);
        ...
        internal virtual async Task<IEnumerable<I>> ExecuteQueryPageListAsync(string predicate, int pageIndex, int pageSize)
        {
            ...
            var qryResult = await QueryableSet().Where(predicate)
        ...
        internal virtual async Task<IEnumerable<I>> ExecuteQueryPageListAsync(Expression<Func<E, bool>> predicate, int pageIndex, int pageSize)
        {
            ...
            var query = await QueryableSet().Where(predicate)
                                            .Skip(pageIndex * pageSize)
                                            .Take(pageSize)
                                            .ToArrayAsync()
                                            .ConfigureAwait(false);
        ...
        internal virtual async Task<IEnumerable<I>> ExecuteQueryAllAsync(string predicate)
        {
            ...
            do
            {
                ...
                qryCount = qryResult.AddRangeAndCount(qry);
