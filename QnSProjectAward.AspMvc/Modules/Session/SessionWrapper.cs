...
using QnSProjectAward.AspMvc.Extensions;
using System;
using System.Threading.Tasks;

namespace QnSProjectAward.AspMvc.Modules.Session
{
    public partial class SessionWrapper : ISessionWrapper
    {
        ...
        public async Task<bool> HasRoleAsync(string role, params string[] further)
        {
            var result = false;
            var loginSession = LoginSession;
            ...
            if (loginSession != null)
            {
                ...
                result = await accMngr.HasRoleAsync(loginSession.SessionToken, role).ConfigureAwait(false);
                for (int i = 0; result == false && i < further.Length; i++)
                {
                    result = await accMngr.HasRoleAsync(loginSession.SessionToken, further[i]).ConfigureAwait(false);
                }
            }
            return result;
        }
