...
using QnSProjectAward.BlazorApp.Models.Modules.Language;
...
namespace QnSProjectAward.BlazorApp.Services.Modules.Language
{
    public sealed partial class TranslatorService : ITranslatorService
    {
        ...
        private string CreatePredicate() => $"AppName.Equals(\"{nameof(QnSProjectAward)}\") && {nameof(KeyLanguage)} == {(int)KeyLanguage} && {nameof(ValueLanguage)} == {(int)ValueLanguage}";
